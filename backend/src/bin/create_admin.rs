...
use backend::models::{NewOrganization, NewUser, OrgSettings, Organization, User};
use dotenvy::dotenv;
...
async fn main() -> Result<()> {
    ...
    let password_hash = Argon2::default()
        ...
        .to_string();
    let org =
        sqlx::query_as::<_, Organization>("SELECT id, name, api_key FROM organizations LIMIT 1")
            .fetch_optional(&pool)
            .await?;
    let org_id = if let Some(o) = org {
        o.id
    } else {
