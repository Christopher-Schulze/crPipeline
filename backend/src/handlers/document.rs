...
pub async fn upload(
    ...
    let base_filename_for_validation = if let Some(f_name) =
        std::path::Path::new(&user_provided_filename)
            .file_name()
            .and_then(|s| s.to_str())
    {
        f_name.to_string()
    ...
    if base_filename_for_validation.is_empty() {
        // Check after potential path stripping
        return HttpResponse::BadRequest()
    ...
    if lower_filename_for_validation.ends_with(".pdf") {
        ...
        if !bytes_data.starts_with(PDF_MAGIC_BYTES) {
            log::warn!(
                "Invalid PDF magic bytes for file '{}'",
    ...
    if detected_file_type.is_none() {
        // Should be caught by else above, but as a safeguard
        return HttpResponse::BadRequest().json(
            serde_json::json!({"error": "Could not determine file type or type is unsupported."}),
        );
    }
