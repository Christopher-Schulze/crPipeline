...
use std::path::Path;
use tokio::process::Command; // For Value type hint
...
pub async fn run_external_ocr(
    ...
    if let Some(key_str) = api_key.filter(|k| !k.trim().is_empty()) {
        if key_str.to_lowercase().starts_with("bearer ") {
            ...
            match HeaderValue::from_str(token_part) {
                Ok(mut header_val) => {
                    // Make mutable to potentially set sensitive
            ...
            // Assume it's a simple key for X-API-KEY or similar
            match HeaderValue::from_str(key_str) {
                ...
                Err(e) => {
                    log::warn!(
                        "Invalid characters in X-API-KEY for external OCR: {:?}. Header not sent.",
                        e
                    );
                }
            }
    ...
    let response = request_builder.send().await.with_context(|| {
        format!(
            "External OCR API request to {} failed to send",
            api_endpoint
        )
    })?;
...
pub async fn run_parse_stage(
    ...
    match config {
        ...
        Some(ParseConfig::SimpleTableExtraction {
            ...
            log::warn!(
                "SimpleTableExtraction is a basic placeholder and may not yield useful results."
            );
            let mut result_data = HashMap::new();
            result_data.insert(
                "status".to_string(),
                JsonValue::String("SimpleTableExtraction (placeholder)".to_string()),
            );
            result_data.insert(
                "matched_headers".to_string(),
                serde_json::to_value(header_keywords)?,
            );
            if let Some(sk) = stop_keywords {
                result_data.insert(
                    "stop_keywords_provided".to_string(),
                    serde_json::to_value(sk)?,
                );
            }
